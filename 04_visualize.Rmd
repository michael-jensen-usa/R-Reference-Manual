...
- Scale Layer
  - Setting x and y limits with scale layer functions, such as `scale_x_continuous()`, that depend on `ggplot2::lims()`, removes data that falls outside those limits and may therefore affect how people perceive the data. Use `coord_cartesian()` instead, which retains all data and uses x and y limits to zoom in.
- `stat_function()`
