...
\hypertarget{htmlwidget-532e93a77fad6985754b}{}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{param_query <-}\StringTok{ }\KeywordTok{dbSendQuery}\NormalTok{(con, }\StringTok{"select * from table where column = ?"}\NormalTok{)}

\KeywordTok{dbBind}\NormalTok{(param_query, input)}
\KeywordTok{dbFetch}\NormalTok{(param_query)}

\CommentTok{# As a function:}
\NormalTok{query_data <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(input) \{}
  
  \KeywordTok{dbBind}\NormalTok{(param_query, input)}
  \KeywordTok{dbFetch}\NormalTok{(param_query)}
\NormalTok{\}}

\KeywordTok{map}\NormalTok{(}\KeywordTok{as.list}\NormalTok{(df}\OperatorTok{$}\NormalTok{input_column), query_data) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{bind_rows}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
