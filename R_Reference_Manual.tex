...
\hypertarget{htmlwidget-2695ad8a4f8c3246d9d0}{}
\begin{grViz}
...
\item
  \texttt{base::array()}
\item
  \texttt{base::c()}: See \texttt{base::vector()}.
\item
  \texttt{base::data.frame()}
\item
  \texttt{base::dir.create()}: Create a file path in the computer's file system.
\item
  \texttt{base::factor()}
\item
  \texttt{base::list()}
...
\item
  \texttt{base::seq()}
\item
  \texttt{base::vector()}: Preferable to \texttt{base::c()} when creating an empty vector (\href{https://www.datacamp.com/community/tutorials/five-tips-r-code-improve}{``Five Tips to Improve Your R Code''}).
\item
  \texttt{stats::rnorm()}
\item
...
\item
  \texttt{tibble::tibble()}
\item
  \texttt{tibble::tribble()}: Row-wise tibble creation.
\end{itemize}
...
\item
  \texttt{base::file.size()}
\item
  \texttt{base::load()}: Reload datasets saved with \texttt{base::save()}.
\item
  \texttt{base::readRDS()}: Restore an R object written with \texttt{base::saveRDS()}.
\item
  \texttt{data.table::fread()}: Similar to \texttt{utils::read.table()}, but faster and more convenient for large data sets.
\item
...
\item
  \texttt{utils::read.table()}: Read a file in table format.
\item
  \texttt{XLConnect::readWorksheetFromFile()}: Read data from worksheets in an Excel file.
\end{itemize}
...
\item
  \texttt{utils::download.file()}: See example below.
\item
  \texttt{utils::unzip()}: See example below.
\end{itemize}
...
\item
  \texttt{DBI::Id()}: See example below.
\item
  \texttt{DBI::SQL()}: See example below.
\item
...
\item
  Try selecting certain columns, rather than \texttt{SELECT\ *}, as SQL BLOB columns cause trouble.
\end{itemize}
...
\hypertarget{dates-and-datetimes}{%
\section{Dates and Datetimes}\label{dates-and-datetimes}}
...
\item
  ...
  \item
    \texttt{ROWS\ \textasciitilde{}\ COLS}: When using a tilde function, the variable on the left specifies the rows and the variable on the right specifies the columns (when faceting in only one direction; i.e., only rows or only columns). Use \texttt{.} to specify nothing for the unused direction (e.g.~\texttt{facet\_grid(cyl\ \textasciitilde{}\ .)}).
  \end{itemize}
\item
