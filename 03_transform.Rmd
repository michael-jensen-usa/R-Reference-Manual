...
- `mean()`
  - Use `base::mean(variable == value)` to calculate proportion; see example below.
- `table()`
...
[`{infer}`](https://rdocumentation.org/packages/infer/versions/1.0.0)
...
- See `{infer}` for easier-to-use alternatives to `{stats}`'s hypothesis testing functions, such as `prop.test()` and `t.test()`.
