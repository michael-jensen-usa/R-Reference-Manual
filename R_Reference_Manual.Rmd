...
* `ggplot2::facet_grid`
  ...
  * `ROWS ~ COLS`: When using a tilde function, the variable on the left specifies the rows and the variable on the right specifies the columns (when faceting in only one direction; i.e., only rows or only columns). Use `.` to specify nothing for the unused direction (e.g. `facet_grid(cyl ~ .)`).
* `ggplot2::facet_wrap`
